...
----------------------------------------
    ...
    def sample(self, batch_size, seq_len, device, pin_memory=False):
        ...
        if self.full:
            if self.pointer - seq_len < 0:
                valid_range = np.arange(self.pointer, self.observation.shape[0] - (seq_len - self.pointer))
            else:
        ...
        sample_idcs = (start_index + seq_indices) % self.observation.shape[0]
    ...
    def __len__(self):
        return self.pointer if not self.full else self.observation.shape[0]
